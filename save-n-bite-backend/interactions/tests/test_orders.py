# interactions/tests/test_orders.py
from django.test import TestCase
from datetime import datetime, timedelta
from interactions.tests.factories import (
    OrderFactory,
    InteractionStatusHistoryFactory,
...
class OrderModelTests(TestCase):
    def setUp(self):
        ...
        self.interaction_item = InteractionItemFactory(
            interaction=self.order.interaction,
            quantity=1
        )
        self.pickup_details = "15:00 - 17:00"
        self.status_history = InteractionStatusHistoryFactory(
            interaction=self.order.interaction
        )

    def test_order_creation(self):
        """Test that an order is created correctly"""
        self.assertEqual(self.order.interaction.interaction_type, 'Purchase')
    ...
    def test_order_items_property(self):
        ...
        self.assertEqual(items.first(), self.interaction_item)
    ...
    def test_status_history_creation(self):
        ...
        self.assertEqual(self.status_history.interaction, self.order.interaction)
    ...
    def test_status_history_str_method(self):
        """Test the __str__ method of InteractionStatusHistory"""
    ...
    def test_status_history_ordering(self):
        ...
        new_status_history = InteractionStatusHistoryFactory(
            interaction=self.order.interaction,
            old_status='confirmed',
            new_status='completed'
        )
        
        history = self.order.interaction.status_history.all()
