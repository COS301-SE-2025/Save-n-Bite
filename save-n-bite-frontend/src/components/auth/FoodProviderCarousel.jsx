    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-colors">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex space-x-2">
          <button
            onClick={prevSlide}
            className="p-2 rounded-full bg-emerald-100 text-emerald-600 hover:bg-emerald-200 dark:bg-emerald-900 dark:text-emerald-300 dark:hover:bg-emerald-800 transition-colors"
          >
            <ChevronLeft size={20} />
          </button>
          <button
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {provider.specialties.slice(0, 2).map((specialty, index) => (
                      <span
                        key={index}
                        className="text-xs px-2 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-200 rounded-full"
                      >
                        {specialty}
                      </span>
                    ))}
                    {provider.specialties.length > 2 && (
                      <span className="text-xs px-2 py-1 bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300 rounded-full">
                        +{provider.specialties.length - 2}
                      </span>
                    )}
                  </div>
                  
                  <button className="w-full py-2 text-sm font-medium text-emerald-600 border border-emerald-600 rounded-md hover:bg-emerald-600 hover:text-white dark:text-emerald-300 dark:border-emerald-300 dark:hover:bg-emerald-800 dark:hover:text-white transition-colors duration-200">
                    View Options
                  </button>
                </div>
              </div>
