...
const Listings = () => {
  ...
  useEffect(() => {
    getListings(search, statusFilter);
  }, [search, statusFilter]);
  ...
  const getListings = async (searchTerm = '', statusFilterValue = 'All') => {
    try {
      setLoading(true);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        ...
        <p className="text-gray-500">
          Manage all food listings on the platform
        </p>
      </div>
      ...
      {showConfirmModal && confirmAction && (
        <ConfirmationModal
          title={
            confirmAction.type === 'remove' ? 'Remove Listing' : 'Flag Listing'
          }
          message={
